...
// TODO: DataView instead of Float32Array & Uint32Array so we can control byteorder...
...
document.cursorPartyWs = document.cursorPartyWs || {
	...
	reconnect: function() {
		...
		if (window.location.hostname.endsWith(".ookie.click")) {
			this.url = `wss://${window.location.hostname}/party/rock?_pk=0&from=cc`;
		}
	...
	queueMyPosition: function() {
		if (this.socket != null && this.socket.readyState == 1 && this.queueMyPositionTimeout == -1) {
			this.queueMyPositionTimeout = setTimeout(() => {
				...
				this.socket.send(new Float32Array([cur[0] / bounds[0], cur[1] / bounds[1]]));
			}, 25);
		}
	},
	...
	handleMessageBinary: function(buffer) {
		...
		for (const _c = u32.length; pos < _c;) {
